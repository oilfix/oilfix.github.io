...
(() => {
  ...
  let token=sessionStorage.getItem(tokenKey)||'',me=null,settings=null,staff=[],adminUsers=[],adminTeam=null,confirmAction=null,confirmNeedsReason=false;
  ...
  function fillBootstrap(data){me=data.user||me;const b=data.bootstrap||data;settings=b.settings||settings;staff=b.staff||staff;adminUsers=b.adminUsers||adminUsers;adminTeam=b.adminTeam||adminTeam;setIdentity();applyRoleVisibility();renderDashboard(b.dashboard);renderLimits();populateSelects();if(me.role==='ADMIN'){renderUserList(adminUsers);fillAdminSettings(b.adminSettings||{});if(adminTeam){fillTeamAdmin(adminTeam);}}}
  ...
  function renderLimits(){if(!settings)return;$('#limitStrip').innerHTML=Object.entries(settings.limits||{}).map(([n,l])=>{const d=Number((settings.discounts||{})[n]||0);return`<div class="limit-item"><span>${esc(n)}${d>0?` · 협약 ${d}% OFF`:''}</span><b>${l}개 / DAY</b></div>`;}).join('');syncSaleLimit();}
  ...
  function syncSaleLimit(){if(!settings)return;const type=$('#saleType').value,limit=Number((settings.limits||{})[type]||1),discount=Number((settings.discounts||{})[type]||0);$('#saleQty').max=String(limit);if(Number($('#saleQty').value)>limit)$('#saleQty').value=String(limit);$('#saleLimitHint').textContent=`${type} · 하루 최대 ${limit}개${discount>0?` · 협약 ${discount}% 할인`:''}`;}
  ...
  function renderQuote(q){$('#qUnit').textContent=shortMoney(q.unitPrice);$('#qUsed').textContent=`${q.used}/${q.limit}개`;$('#qRemain').textContent=`${Math.max(0,q.remainingBefore)}개`;$('#qTotal').textContent=shortMoney(q.total);let msg=q.goldenBell?'골든벨 무료 · 0원 · 후불 제외':q.deferred?`${q.customerType} 후불 · ${q.customerType==='경찰'?'월요일':'일요일'} 정산`:'즉시 결제';if(Number(q.discountRate||0)>0&&!q.blacklist&&!q.goldenBell)msg=`협약 ${q.discountRate}% 할인 적용 · ${shortMoney(q.basePrice)} → ${shortMoney(q.unitPrice)} · ${msg}`;if(q.blacklist&&!q.goldenBell)msg=`블랙리스트 가격 적용 · 협약 할인 제외 · ${msg}`;if(!q.allowed)msg=`한도 초과 · 추가 가능 ${q.remainingBefore}개`;if(q.duplicate)msg+=' · ⚠ 최근 동일 거래 감지';$('#qMessage').textContent=msg;$('#qMessage').style.color=q.allowed?'':'var(--danger)';}
  $('#saleForm').addEventListener('submit',async e=>{e.preventDefault();const btn=$('#saleSubmit');busy(btn,true);try{const q=await gas('apiQuoteSale',token,salePayload());renderQuote(q);if(!q.allowed)throw new Error(`구매 한도 초과입니다. 추가 가능 ${q.remainingBefore}개`);openConfirm('이 거래를 등록할까요?',`${me.name} → ${q.nickname}(#${q.gameId}) · ${q.customerType} · ${q.quantity}개 · ${shortMoney(q.total)}${q.goldenBell?' · 골든벨 무료':(Number(q.discountRate||0)>0?' · 협약 '+q.discountRate+'% 할인':'')}`,false,async()=>createSale(false));}catch(err){handleError(err);}finally{busy(btn,false);}});
  ...
  $('#teamMetaForm').addEventListener('submit',async e=>{e.preventDefault();try{const r=await gas('apiAdminSaveTeamMeta',token,{current:Number($('#teamCurrent').value),capacity:Number($('#teamCapacity').value)});adminTeam=r.team;fillTeamAdmin(adminTeam);toast(r.message);}catch(e){handleError(e);}});
  $('#teamMemberForm').addEventListener('submit',async e=>{e.preventDefault();try{const r=await gas('apiAdminSaveTeamMember',token,{id:$('#teamEditId').value,tier:$('#teamTier').value,position:$('#teamPosition').value.trim(),name:$('#teamName').value.trim(),gameId:$('#teamGameId').value.trim(),sortOrder:Number($('#teamSortOrder').value||0),vacant:$('#teamVacant').checked});adminTeam=r.team;fillTeamAdmin(adminTeam);resetTeamForm();toast(r.message);}catch(e){handleError(e);}});
  $('#teamFormReset').addEventListener('click',resetTeamForm);
  $('#teamRefreshBtn').addEventListener('click',loadAdminTeam);
  async function loadAdminTeam(){if(!me||me.role!=='ADMIN')return;try{const r=await gas('apiAdminGetTeam',token);adminTeam=r.team;fillTeamAdmin(adminTeam);}catch(e){handleError(e);}}
  function fillTeamAdmin(team){if(!team)return;$('#teamCurrent').value=team.current??0;$('#teamCapacity').value=team.capacity??25;renderTeamAdminList(team.rows||[],team);}
  function resetTeamForm(){$('#teamEditId').value='';$('#teamTier').value='MANAGER';$('#teamPosition').value='매니저';$('#teamName').value='';$('#teamGameId').value='';$('#teamSortOrder').value='10';$('#teamVacant').checked=false;syncTeamVacant();}
  function syncTeamVacant(){const v=$('#teamVacant').checked;$('#teamGameId').disabled=v;if(v&&!$('#teamName').value.trim())$('#teamName').value='공석';}
  $('#teamVacant').addEventListener('change',syncTeamVacant);
  function renderTeamAdminList(rows,team){const box=$('#teamAdminList');if(!rows||!rows.length){box.className='team-admin-list empty-state';box.innerHTML='직급표 항목이 없습니다.';return;}box.className='team-admin-list';box.innerHTML=`<div class="team-admin-summary"><span>공식 홈페이지 표시 인원</span><strong>${Number(team.current||0)} / ${Number(team.capacity||0)}</strong></div>`+rows.map(x=>`<div class="team-admin-row ${x.vacant?'vacant':''}"><div class="team-tier">${esc(x.tier)}</div><div><b>${esc(x.position)} · ${esc(x.name||'공석')}</b><span>${x.vacant?'OPEN':('#'+esc(x.gameId))} · 정렬 ${x.sortOrder}</span></div><div class="team-admin-actions"><button class="ghost small" data-edit-team="${esc(x.id)}">수정</button><button class="ghost small" data-delete-team="${esc(x.id)}">삭제</button></div></div>`).join('');}

  $('#settingsForm').addEventListener('submit',async e=>{e.preventDefault();try{const r=await gas('apiAdminSaveSettings',token,{BASE_PRICE:$('#setBasePrice').value,BLACKLIST_MULTIPLIER:$('#setBlackMultiplier').value,DISCOUNT_POLICE:$('#setDiscountPolice').value,DISCOUNT_SECURITY:$('#setDiscountSecurity').value,DISCOUNT_GOLDCASH:$('#setDiscountGoldcash').value,LIMIT_GENERAL:$('#setLimitGeneral').value,LIMIT_POLICE:$('#setLimitPolice').value,LIMIT_SECURITY:$('#setLimitSecurity').value,LIMIT_GOLDCASH:$('#setLimitGoldcash').value,LIMIT_GOLDMOON:$('#setLimitGoldmoon').value,DUPLICATE_WINDOW_SECONDS:$('#setDuplicateSec').value});settings=r.publicSettings;fillAdminSettings(r.values);renderLimits();toast(r.message);}catch(e){handleError(e);}});
  function fillAdminSettings(v){if(!v||!Object.keys(v).length)return;$('#setBasePrice').value=v.BASE_PRICE||'';$('#setBlackMultiplier').value=v.BLACKLIST_MULTIPLIER||'';$('#setDiscountPolice').value=v.DISCOUNT_POLICE??'50';$('#setDiscountSecurity').value=v.DISCOUNT_SECURITY??'50';$('#setDiscountGoldcash').value=v.DISCOUNT_GOLDCASH??'50';$('#setLimitGeneral').value=v.LIMIT_GENERAL||'';$('#setLimitPolice').value=v.LIMIT_POLICE||'';$('#setLimitSecurity').value=v.LIMIT_SECURITY||'';$('#setLimitGoldcash').value=v.LIMIT_GOLDCASH||'';$('#setLimitGoldmoon').value=v.LIMIT_GOLDMOON||'';$('#setDuplicateSec').value=v.DUPLICATE_WINDOW_SECONDS||'';}
  $$('[data-admin-tab]').forEach(btn=>btn.addEventListener('click',()=>{$$('[data-admin-tab]').forEach(x=>x.classList.remove('active'));btn.classList.add('active');const tab=btn.dataset.adminTab;$('#adminUsersPanel').classList.toggle('hidden',tab!=='users');$('#adminTeamPanel').classList.toggle('hidden',tab!=='team');$('#adminSettingsPanel').classList.toggle('hidden',tab!=='settings');if(tab==='team')loadAdminTeam();}));
  ...
  document.addEventListener('click',e=>{
    ...
    const et=e.target.closest('[data-edit-team]');if(et){const x=(adminTeam&&adminTeam.rows||[]).find(r=>r.id===et.dataset.editTeam);if(!x)return;$('#teamEditId').value=x.id;$('#teamTier').value=x.tier;$('#teamPosition').value=x.position;$('#teamName').value=x.name;$('#teamGameId').value=x.gameId||'';$('#teamSortOrder').value=String(x.sortOrder||0);$('#teamVacant').checked=!!x.vacant;syncTeamVacant();document.querySelector('#admin').scrollIntoView({behavior:'smooth'});return;}
    const dt=e.target.closest('[data-delete-team]');if(dt){openConfirm('직급표 항목 삭제','공식 홈페이지 직급표에서 이 항목을 삭제할까요?',false,async()=>{try{const r=await gas('apiAdminDeleteTeamMember',token,dt.dataset.deleteTeam);closeConfirm();adminTeam=r.team;fillTeamAdmin(adminTeam);toast(r.message);}catch(err){handleError(err);}});return;}
